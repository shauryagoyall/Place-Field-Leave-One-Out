...
if isempty(varargin)
    place_fields_odd = importdata('extracted_place_field_laps_odd.mat');
    place_fields_even = importdata('extracted_place_field_laps_even.mat');
else
    place_fields_odd= cell2mat(varargin);
end
...
for kk=3:4
    for j=1:2
        y_vector=[];
        color_index=1+floor(place_fields_even.track(kk).peak*2);
        color_index(find(color_index>64))=64;
        field_color=colormap(jet);
        for ii=1:length(place_fields_odd.track(j).sorted_good_cells)
            %plot sorted
            matrix=[];
            normalized_matrix=[];
            matrix(ii,:)=place_fields_even.track(kk).smooth{place_fields_even.track(j).sorted_good_cells(ii)};
            normalized_matrix(ii,:)=(matrix(ii,:)-min(matrix(ii,:)))/(max(matrix(ii,:))-min(matrix(ii,:)));
            subplot(length(place_fields_odd.track)-2,length(place_fields_odd.track)-2,c)
        ...
        yt=place_fields_odd.track(j).sorted_good_cells;
        set(gca,'ytick',y_vector);
        set(gca,'yticklabel',yt);
        ylabel('Unit ID');
        xlabel('sorted linearized position (bins)');
        c=c+1;
        title([{['place cells on even track ' num2str(kk)]} ; {['sorted by even track ' num2str(j)]}]);
    end
end
